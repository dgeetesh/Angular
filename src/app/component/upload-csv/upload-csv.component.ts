...
export class UploadCsvComponent implements OnInit {
  ...
  constructor(private router: Router,private formBuilder: FormBuilder, private commonService: CommonService, private location: Location ) { }
...
uploadOnChange(event){
  console.log('event.target.fields',event.target.value);
  this.file =event.target.files[0];
}
  ...
  if(this.file) 
    {
      ...
      this.commonService.uploadImage("api/users/uploadPost", value ,TokenHeader)
      .subscribe((resp) => {
